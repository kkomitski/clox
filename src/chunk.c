...
int addConstant(Chunk *chunk, Value value)
{
  ...
  // The dot syntax `.` is for accessing struct members directly from a struct variable
  return chunk->constants.count - 1;

  /*
    The `writeValueArray` function will end up incrementing the count of the
    ValueArray struct internally, so we return count-1 to return the index of the item we actually added
  */
}